...
export default function MissionArea({
  ...
  const missionData = hasLabs
    ? labs.map((lab) => ({
      ...
      imgSrc: lab.picture?.url ? (lab.picture.url.toLowerCase().startsWith("http") ? lab.picture.url : `${process.env.NEXT_PUBLIC_API_BASE_URL}${lab.picture.url}`) : mission_thumb_1,
    }))
    : fallbackMissionData;
